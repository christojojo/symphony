...
//   useEffect(() => {
//     const mediaQuery = window.matchMedia("(min-width: 769px)"); // Change the condition to min-width
...
function EventDesign() {
  ...
  useEffect(() => {
    const mediaQuery = window.matchMedia("(min-width: 1024px)"); // Change the condition to min-width
    if (mediaQuery.matches) {
      const context = gsap.context(() => {
        ScrollTrigger.create({
          trigger: boxRef.current,
